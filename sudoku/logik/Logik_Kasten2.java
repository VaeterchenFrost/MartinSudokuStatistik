...
class Logik_Kasten2 extends LogikKastenN{
	...
	public String gibErgebnis() {
		return "Im Kasten wird die Zahl auf diesen 2 Linien aus den möglichen dieser Felder gelöscht.";
	}
		...
		for (int basisZahl=1; basisZahl<10; basisZahl++){
			...
			if (gleichePaarLinien){
				...
				for (int iLinieDerZahl=0; iLinieDerZahl < iLinien0DerZahl.size(); iLinieDerZahl++){
					...
					KastenErgebnis ergebnis1 = new KastenErgebnis(
						istSpalte, basisZahl, 
						new FeldNummerListe(kannLoeschenIn), 
						nachbarUrsacheFelder); 
				...
				boolean gibtEsLoeschFelder = (! ergebnis0.gibLoeschFelder().isEmpty()) | (! ergebnis1.gibLoeschFelder().isEmpty()); 
