...
abstract class LogikKastenN implements Logik__Interface{
	...
	protected class KastenErgebnis {
		...
		public KastenErgebnis(boolean istSpalte, 
				int loeschZahl, 
				FeldNummerListe geloeschtInFelder, 
	...
	private class TipInfoKasten  extends TipInfo0{
		 ...
		 * @param infoSudoku
		 */
		private TipInfoKasten(Logik_ID logik,
				FeldNummerListe mitSpieler, 
				String textInGruppe,
				KastenErgebnis ergebnis) 
		{
		...
		private EinTipText gibTipKasten1(KastenErgebnis ergebnis1){
			...
			String s1 = String.format("%s wird die %d gelöscht in %s. Grund:", 
					this.textInGruppe,
					ergebnis1.gibLoeschZahl(),
					geloeschtIn);
			String s2 = String.format("Der %s benötigt die %d in %s.", 
					kastenUrsache,
					ergebnis1.gibLoeschZahl(),
					sUrsache);
		...
		private EinTipText gibTipKasten2(KastenErgebnis ergebnis1){
			...
			String s1 = String.format("%s wird die %d gelöscht in %s.  Grund:", 
					this.textInGruppe, ergebnis1.gibLoeschZahl(), ergebnis1.gibLoeschFelder().gibKette("+"));

			String inText = ergebnis1.istSpalte() ? "in den Spalten": "in den Zeilen";
			String s2 = String.format(" %s und %s benötigen die %d %s %s.", 
					sNachbar1, sNachbar2, ergebnis1.gibLoeschZahl(), inText, sLinien);
			return new EinTipText(s1, s2);
		}
	...
	private boolean istErgebnisIgnorieren(List<TipInfo> ignorierTips, TipInfoKasten tipInfoKasten){
		...
		for(TipInfo tipInfo: ignorierTips){
			...
			boolean gleicheLoeschZahlen = ignorierTip.gibLoeschZahlen().istGleicherInhalt(tipInfoKasten.gibLoeschZahlen());
