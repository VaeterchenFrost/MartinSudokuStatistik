...
public class SudokuLogik extends FeldMatrix {
	...
	static private EnumMap<Logik_ID, Logik__Infos> gibLogiken() {
		...
		for (int iLogik = 0; iLogik < logikArray.length; iLogik++) {
			...
			case KASTEN2:
				...
				break;
			// case KASTENLINIE:
			// logikMap.put(logikArray[iLogik], new Logik_KastenLinie(null,
			// null, null));
			// break;
			case FELD:
			...
			case AUSWIRKUNGSKETTE:
				...
				break;
			default:
				// F�r eine Exception ist es zu dieser (static) Stunde noch ein
				// bischen fr�h. Deshanlb dies unten.
	...
	// * @return Startnummer (ab 1) der Gruppe des Gruppentyps innerhalb aller
	// Gruppen(-Typen)
	// */
	...
	public SudokuLogik(EintragsEbenen eintragsEbenen) {
		...
		// Diese Reihenfolge soll der Reihenfolge in Gruppe.Typ entsprechen: F�r
		// die L�sungs-Zeit-Errechnung!
	...
	/**
	 * Solange freie Felder mit nur einer m�glichen Zahl erkannt werden,
	 * erhalten diese ihren Eintrag. Das Sudoku muss danach nicht gel�st sein:
	 * Wenn es n�mlich kein eindeutiges ist!
	 * 
	 * @throws Exc
	 * @return Problem falls das Sudoku zuvor einen falschen Eintrag erhielt,
	 *         sonst null.
	 */
	public Problem setzeEintraegeAufKlare(Klugheit klugheit, boolean alle, boolean feldPaareSetzen) throws Exc {
		do {
			SetzeMoeglicheErgebnis ergebnis = this.setzeMoegliche(klugheit, feldPaareSetzen);
	...
	/**
	 * Vermerkt in den Feldern wenn es in einer Gruppe genau 2 Felder gibt mit
	 * ein und derselben Zahl.
	 * 
	 * @throws Exc
	...
	/**
	 * Ziel ist das Erkennen bzw. Erreichen eines freien Feldes mit nur einer
	 * m�glichen Zahl: Dass kann dann ein Eintrag werden. F�r freie Felder
	 * werden die m�glichen Zahlen neu ermittelt: 1. Jedes Feld setzt seine
	 * m�glichen Zahlen 2. Soweit es die Logik hergibt werden von diesen Zahlen
	 * gel�scht, um Felder mit nur einer m�glichen Zahl zu erreichen. 3. Es
	 * werden Probleme gesucht.
	 * 
	 * @param feldPaareSetzen
	 *            Mit true werden die in jedem Feld gesetzen FeldPaare
	 *            angefordert
	 * @return SetzeMoeglicheErgebnis: Mit einem von 4 denkbaren Zust�nden: 1.
	 *         Mit dem zu setzenden Eintrag oder 2. Mit einem erkannten Problem
	 *         wenn das Sudoku eine falsche Zahl beinhaltet oder - Leeres
	 *         Ergebnis 3. wenn die L�sung nicht weiter vorangetrieben werden
	 *         kann oder 4. das Sudoku bereits vollst�ndig gel�st war.
	 * @throws Exc
	 */
	public SetzeMoeglicheErgebnis setzeMoegliche(Klugheit klugheit, boolean feldPaareSetzen) throws Exc {
		...
		// Jedes freie Feld ermittelt die M�glichen anhand seiner Gruppe
		// (Zeile+Spalte+Kasten) neu
		...
		while (true) {
			...
			if (null != eintrag) {
				// Das Erkennen eines freien klaren Feldes (mit nur einer
				// m�glichen Zahl) war das Ziel
				break;
			}
			...
			for (int iLogik = 0; iLogik < logiken.length; iLogik++) {
				...
				if (klugheit.istSoKlug(logik.gibLogikID())) {
					...
					// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
					LogikErgebnis logikErgebnis = logik.laufen(tipKurier != null, ignorierTips);
					// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

					if (logikErgebnis != null) {
						// Tip setzen
						if (tipKurier != null) {
							// Es gibt Logikl�ufe, die gar keine entsprechende
							// Logik-Situation vorfinden!
						...
						// Das L�schen der vorgeschlagenen m�glichen Zahlen
						// realisieren
						...
						if (logikErgebnis.eintrag != null) {
							...
							// Ein f�r einen Eintrag vorgesehenes Feld besitzt
							// im Allgemeinen mehr als nur eine m�gliche Zahl:
							// Z.B. sieht Logik O1 in einer Gruppe die 8 einzig
							// im einem Feld.
							// Da k�nnen in diesem Feld noch andere m�gliche
							// Zahlen drin sein.
							// Es muss noch daf�r gesorgt werden, dass dieses
							// Feld nur einzig die Eintrags-Zahl
							// als m�gliche Zahl besitzt:
							Feld feld = this.gibFeld(eintrag.gibFeldNummer());
							feld.setzeMoeglichEinzig(eintrag.gibZahl());

							// Ein gefundene Zahl, die als Eintrag im folgenden
							// gesetzt werden kann:
						...
						if (logikErgebnis.loeschZahlen != null) {
							...
							if (anzahlGeloeschterZahlen > 0) {
								// Nach gel�schten Zahlen soll wieder ab der
								// einfachsten Logik begonnen werden
								break; // for
							}
						}
					} // if (logikErgebnis != null) {
