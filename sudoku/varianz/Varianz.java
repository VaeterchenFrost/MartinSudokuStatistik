...
public class Varianz {
	 ...
	 * @param loesungen
	 *            Bei loesungen==null wird die erste L�sung per Exception
	 *            "SudokuFertig" gemeldet. Das sudoku enth�lt dann diese L�sung.
	 * @param maxAnzahl
	 *            Ist nur relevant bei loesungen != null: Ist die maximale
	 *            Anzahl L�sungen, die gew�nscht werden.
	 * @throws Exc
	 * @throws SudokuFertig
	 */
	static private void sucheLoesungen(SudokuLogik sudoku, Klugheit klugheit, ProtokollMarkierer protokollMarkierer,
		 ...
		 * Problem => unl�sbar 1. c) unfertig => 2. 2. Mithilfe Protokoll und
		 * den Ebenen st�ckweise vorgehen: Alles abbrechbar. Mit allen freien
		 * Feldern, mit allen M�glichen: M�gliche Zahl setzen +
		...
		try {
			...
			while (true) {
				...
				if (ergebnis.gibProblem() != null) {
					// Das Sudoku ist versaut: F�r diesen Zustand gibt es keine
					// L�sung
				...
				if (!sudoku.istUnFertig()) {
					// Fertig => L�sung melden oder eintragen
					if (loesungen == null) {
				...
				if (ergebnis.gibEintrag() == null) {
					// F�r diesen Zustand gibt es keine logisch eindeutige
					// L�sung (mit der Klugheit)
					protokollMarkierer.markierungAnsteuern(markierungStart);
					break; // =======================> unten weiter
				}
