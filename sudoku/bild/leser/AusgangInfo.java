...
//	}
	...
	static private Point gibStart(BufferedImage image, Rectangle rechteck, boolean istOben, boolean istSystemOut){
		...
		if (istSystemOut){
			String s = String.format("gibStart: oben=%d xStart=%d yStart=%d Rechteck=%s", 
					istOben ? 1 : 0,  x, y, rechteck);
			System.out.println(s);
		}
	 ...
	 */
	static private Point gibSackgasseAusgang(BufferedImage image, int x, int y, int xEnde, int yAusweich, boolean istSystemOut){
		int xRichtung = 1;
		if(xEnde < x){
			xRichtung = -1;
		}
		...
		if (istSystemOut){
			String s = String.format("gibSackgasseAusgang-Start: x=%d y=%d xEnde=%d yAusweich=%d xRichtung=%d", 
					x, y, xEnde, yAusweich, xRichtung);
			System.out.println(s);
		}
	...
	static private boolean istAusgang(BufferedImage image, Point start, int xZiel, int yAusweich, boolean istSystemOut){ 
		...
		if (istSystemOut){
			String s = String.format("istAusgang: xStart=%d yStart=%d xZiel=%d xRichtung=%d yAusweich=%d", 
					start.x, start.y, xZiel, xRichtung, yAusweich);
			System.out.println(s);
		}
	 ...
	 */
	static private Ausgang gibAusgang(BufferedImage image, Rectangle rechteck, boolean istOben, Point start, boolean istSystemOut){
