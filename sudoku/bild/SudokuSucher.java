...
public class SudokuSucher { // implements LangerProzess{
	...
	// System.out.println(String.format("%s-Weiss (/10) bis Index %d
	// (maxIndex=%d)", titel, iMax, intWeiss.length));
	// if (istSystemOut) {
	// char zeichen = '#';
	...
	// /**
	// * Es wird vorausgesetzt, dass nur eines von beiden OK ist: Spalten oder
	// Zeilen
	// *
	// * @param image
	// * @param spaltenStriche
	...
	// */
	// static private Rectangle gibBesserenBildAusschnitt(BufferedImage image,
	 ...
	 *            stricheKreuz mit nur senkrechten oder waagerechten Strichen
	 *            (die wurden schon erfolgreich erkannt). Die bereits erkannten
	 *            Striche der einen Richtung werden �bernommen. Sie werden nicht
	 *            neu erstellt!
	...
	public void prozess() {
		...
		// Das hilft bei Bildern mit schiefen Sudokus oder auch bei miesen
		// Bildern
	 ...
	 * 
	 * @param linienWeiss
	 *            Info zu den Weiss-Anteilen der Spalten bzw. Zeilen
	 * @param nSuchLauf
	 *            Eine gr��ere Zahl verkleinert die geforderte Strichl�nge
	...
	private boolean suchen(LinienWeiss linienWeiss, int nSuchLauf) {
		...
		// Es wird davon ausgegangen, dass auf der schmalen Bildseite max 3
		// Sudokus liegen,
		// auf der langen Seite max 6 Sudokus.
		// Die Zwischenr�ume zwischen den Sudokus ber�cksichtigen heisst:
		// jeweils + 1.
		...
		if (stricheKreuz.istOK()) {
			...
			// Es wird vorausgesetzt, dass s�mtliche Striche richtige
			// Sudoku-Striche
			...
			// Das Sudoku muss schon ordentlich gro� auf dem Monitor sein, damit
			// die Zahlen sicher erkannt werden.
			...
			//
			// // Im Bildausschnitt ist der Kick, dass die zu erkennenden
			// Striche alle sehr lang sind:
			// int minStrichLaenge = 50;
			// //stricheKreuz: die bekannten Striche m�ssen jetzt auf den
			// Ausschnitt transformiert werden!
			// stricheKreuz.transformiereIndizees(bessererBildAusschnitt.getLocation());
			//
			//
			// StricheKreuz stricheKreuz2 = gibStriche(besseresBild,
			// stricheKreuz, minStrichLaenge, minStrichLaenge,
			// "Bildausschnitt");
			//
