...
/**
 * @author heroe Der Pool zur Aufbewahrung von Sudokus in Form von
 *         InfoSudoku-Dateien
 */
public class DateiPool implements Pool0 {
	 ...
	 * @param loesungsZeit
	 * @return Den durch eine weitere Nummer erg�nzten Dateinamen wenn noch
	 *         nicht anzahlJeMinute Dateien f�r die L�sungszeit existieren,
	 *         sonst null
	 */
	...
	/**
	 * @param neuTyp
	 * @return Vorgaben des angeforderten Typs. Leere Vorgaben bei nicht zu
	 *         erstellendem Typ
	...
	public Boolean setze(NeuTyp neuTyp, InfoSudoku sudoku, int loesungsZeit) {
		...
		// LogiRex stark begrenzen: Nur gerade L�sungszeiten speichern
		...
		synchronized (this) {
			...
			if (vollTyp.equals(neuTyp)) {
				...
				if (f.exists()) {
					// LogiRex stark begrenzen: Je L�sungszeit nur eine Datei
					// speichern
					if (neuTyp.equals(new NeuTyp(Schwierigkeit.LOGIREX))) {
						return null;
					}
